...
for i, threshold in enumerate(thresholds):
    ...
    for train_index, test_index in logo.split(final_df, final_df['label'], final_df['group_loci']):
        ...
        df_preds = pd.DataFrame({
            ...
            'protein_index': test_df['protein_index'],
            'protein_ID': test_df['protein_ID'],
            'true_label': y_test,
            'score': score_xgb
        })

        max_score_per_pair = df_preds.loc[df_preds.groupby(['accession', 'phage_ID'])['score'].idxmax()]
        max_score_per_pair['threshold'] = tstr[i]

        max_score_pairs.append(max_score_per_pair)

        max_scores = max_score_per_pair[['score', 'true_label']]
